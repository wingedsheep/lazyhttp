...
> [!WARNING]
> **A `.http` plan can run arbitrary commands.** The `@shell` directive runs its
> body through your shell, and folder mode auto-discovers every plan under a
> directory — so opening and running a plan from an untrusted source executes
> whatever commands it contains, with your privileges. Treat a `.http` file like a
> script: only run plans you've read or trust, exactly as you would `curl … | sh`.
> The same applies to `lazyhttp run` in CI — point it at plans from your own repo,
> not ones a pull request can introduce.
